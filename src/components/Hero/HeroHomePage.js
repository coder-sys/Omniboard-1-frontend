...
const [date_err,setDR] = useState(0)
  const Delete = async(foldername)=>{
    ...
    let api = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/delete_folder/${name+emailandlastname['lastname']+emailandlastname['email']}/${foldername}`)
    api = await api.json()
    console.log(api.status)
    let api1 = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/get_folders/${name+emailandlastname['lastname']+emailandlastname['email']}`)
  api1 = await api1.json()
  console.log(api.data)
  setFolderData(api1.data)
  console.log(true,name+emailandlastname['lastname']+emailandlastname['email'])
  setUpdated()
  let api2 = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/delete_no_of_folders/${name}`)
  api2 = await api2.json()
  }
  
const UE = async()=>{
  let date_error = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/date_subtraction_for_paid_version`)
...
let statsapi = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/get_no_of_stored_content/${name+emailandlastname['lastname']+emailandlastname['email']}/${api1.data.join('-')}`)
console.log(statsapi.status)
console.log(`https://espark-api-proxy.ecsbeats.repl.co/get_no_of_stored_content/${name+emailandlastname['lastname']+emailandlastname['email']}/${api1.data.join('-')}`)
...
const fetchData_teacher = async () =>{
  let emailandlastname = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/get_last_name_and_email/${name}`)
emailandlastname = await emailandlastname.json()
let student_data_1 = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/view_student_data_alph_order/student`)
...
student_data_1 = await student_data_1.json()
setStudentGraphData(student_data_1['graph_data'])
let student_data_2 = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/view_student_data_alph_order/teacher`)
