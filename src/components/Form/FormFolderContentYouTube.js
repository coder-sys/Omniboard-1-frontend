			...
			<Container>
				<FormRow>
					<FormColumn small>
                       ...
                       let api = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/get_youtube_data/${props.youtubesearch}`)
                       api = await api.json()
                       ...
                       props.translateLink(api.link).then((data)=>{
                        props.translateLink(props.linkarray).then(async(data1)=>{
                               ...
                               console.log(`https://espark-api-proxy.ecsbeats.repl.co/find_similarity_links/${data.join()}/${data1.join()}`)
                               let api = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/find_similarity_links/${data.join()}/${data1.join()}`)
                               api = await api.json()
