      ...
      emailandlastname = await emailandlastname.json()
                ...
                youtubeAPITitles[index].split('').map((data)=>{
                    ...
                    yttitlejoin.push(data)
                                    ...
                                    let api = await fetch(`https://espark-api-proxy.ecsbeats.repl.co/add_youtube_content/${name+emailandlastname['lastname']+emailandlastname['email']}/${foldername}/${yttitlejoin.join('')}/${ytlinkjoin_.join("").split('=')[1]}/${thumbnailjoin_.join('')}`)
                                    api = await api.json()
